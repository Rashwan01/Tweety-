      <div class="flex mt-3">
          <form action="{{ route("like",$tweet->id) }}" method="post">
              ...
              <div class="flex items-center mr-2">
                  <svg viewBox="0 0 20 20"
                      class="{{ $tweet->isLikedBy(current_user()) ? 'text-blue-500' : 'text-gray-600' }}  w-3 mr-1">
                  ...
                  <button type="submit" class="text-gray-600 text-sm">{{ $tweet->likes?:0 }}</button>
              </div>
          </form>
          <form action="{{ route("dislike",$tweet->id) }}" method="post">
              ...
              <div class="flex items-center mr-2">
                  <svg viewBox="0 0 20 20"
                      class="{{ $tweet->isDislikeBy(current_user()) ? 'text-blue-500' : 'text-gray-600' }} w-3 mr-1">
                      <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                          <g class="fill-current">
                              <path
                  ...
                  <button type="submit" class="text-gray-600 text-sm">{{ $tweet->dislikes?:0 }}</button>
              </div>
          </form>
      </div>
